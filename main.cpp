...
void Initialize()
{
	...
	// Leemos el código fuente del vertex shader con la clase
	// auxiliar InputFile
	...
	// Hacemos un cast (conversión de tipo de dato) porque no
	// le podemos mandar strings a OpenGL. Necesitamos mandarle 
	// const GLchar*.
	...
	glGetShaderiv(vertexShaderHandle, GL_COMPILE_STATUS,
	...
	if (vertexShaderCompileSuccess == GL_FALSE)
	{
		...
		if (logLength > 0)
		{
			...
			glGetShaderInfoLog(vertexShaderHandle, logLength, &logLength,
				&compileLog[0]);
	...
	// Asociamos el indice del buffer (VBO) de colores con el nombre
	// de la variable correspondiente en el shader.
